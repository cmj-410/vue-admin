...
import {utils as XLSXutils, SSF as XLSXSSF, write as XLSXwrite} from 'xlsx'
...
function sheet_from_array_of_arrays(data, opts) {
  ...
  for (var R = 0; R != data.length; ++R) {
    for (var C = 0; C != data[R].length; ++C) {
      ...
      var cell_ref = XLSXutils.encode_cell({
        c: C,
        r: R
      })
  ...
  if (range.s.c < 10000000) ws['!ref'] = XLSXutils.encode_range(range)
  return ws
}
...
export const export_json_to_excel = ({
  ...
  if (merges.length > 0) {
    ...
    merges.forEach((item) => {
      ws['!merges'].push(XLSXutils.decode_range(item))
    })
  }
  // 8. 单元格宽度相关
  if (autoWidth) {
